...
# ================== 2. НОРМАЛИЗАЦИЯ НАЗВАНИЙ (словарь для NBA) ==================
TEAM_SYNONYMS = {
    'atlanta hawks': 'hawks',
...
SHORT_TO_FULL = {
    ...
    'rockets': ['houston'],
    'pacers': ['indiana'],
    'clippers': ['la', 'los angeles'],
    'lakers': ['la', 'los angeles'],
    'grizzlies': ['memphis'],
    'heat': ['miami'],
    'bucks': ['milwaukee'],
...
def normalize_team_name(name):
    """Приводит название команды к единому ключу (например, 'lakers')."""
    name = name.lower().strip()
    # Убираем лишние слова и символы
    name = re.sub(r'\b(basketball|club|team|nba)\b', '', name)
...
# ================== 3. ПАРСИНГ FOREBET (NBA) ==================
def get_forebet_matches():
    target_url = 'https://www.forebet.com/en/basketball/usa/nba'
    scraper_url = f"http://api.scraperapi.com?api_key={API_KEY}&url={target_url}"
    
    print("⏳ Парсинг Forebet (NBA)...")
    ...
    print(f"✅ Найдено матчей на Forebet (NBA): {len(forebet_data)}")
    return forebet_data

# ================== 4. ПОЛУЧЕНИЕ РЫНКОВ POLYMARKET (NBA) ==================
def get_polymarket_markets():
    print("\n⏳ Поиск матчей NBA на Polymarket...")
    markets_url = "https://gamma-api.polymarket.com/markets"
    params = {
        "tag_id": "100639",  # ← Официальный тег NBA
        "active": "true",
        "closed": "false",
        "order": "volume24hr",
        "ascending": "false",
    ...
    print(f"✅ Найдено рынков NBA: {len(polymarket_data)}")
...
def analyze_and_display(forebet_matches, polymarket_markets):
    print("\n" + "=" * 80)
    print("РЕЗУЛЬТАТЫ СРАВНЕНИЯ (NBA)")
    print("=" * 80)
